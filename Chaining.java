...
public class Chaining {
    ...
    public int insertKey(int key){
        int slot = chain(key);
        //Find number of collisions (size of chain) BEFORE inserting key into slot
        int collisions = Table.get(slot).size();
        Table.get(slot).add(0, key);
        return collisions;
    }
